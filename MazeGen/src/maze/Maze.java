...
public class Maze {
	...
	private boolean[][] visited; //this is used for generation algorithm and for the solving algorithm
	...
	// solve the maze using depth first search
		private void solve(int x, int y, boolean draw) {
			//these are the walls of the maze, maze solver should not go beyond walls
			...
			if (foundTarget || visited[x][y]) return;
			...
			if (!north[x][y]) solve(x, y + 1, draw);
			if (!east[x][y]) solve(x + 1, y, draw);
			if (!south[x][y]) solve(x, y - 1, draw);
			if (!west[x][y]) solve(x - 1, y, draw);
		...
		/**
		 * Solve the maze starting from Point <start>
		 * and ending at Point <end>
		 * @param start
		 * @param end
		 */
		public void solve(Point2D.Double start, Point2D.Double end, boolean draw) {
			this.solve(new Point((int)start.getX(), (int)start.getY()), new Point((int)end.getX(), (int)end.getY()), draw);
		}
		...
		/**
		 * Returns whether a wall exists in the direction
		 * given from the coordinates (x,y)
		 * @param direction
		 * @param x
		 * @param y
		 * @return
